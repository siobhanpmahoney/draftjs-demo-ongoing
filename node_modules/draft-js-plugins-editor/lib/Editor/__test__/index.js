...
var TestEditor = function (_Component) {
  ...
  _createClass(TestEditor, [{
    ...
    value: function componentWillReceiveProps(props) {
      this.setState({
        editorState: (0, _index.createEditorStateWithText)(props.text)
      });
    }
  }, {
    key: 'render',
...
describe('Editor', function () {
  ...
  describe('decorators prop', function () {
    ...
    it('uses components from both decorators and plugins together', function () {
      ...
      var decoratorComponents = wrapper.find('.decorator');
      var pluginComponents = wrapper.find('.plugin');
    ...
    it('reassigns decorators to editorState when props are updated with naked editorState', function (done) {
      var props = { plugins: plugins, text: text };
      var comp = (0, _enzyme.mount)(_react2.default.createElement(TestEditor, props));

      var decoratorNumber = comp.state('editorState').getDecorator().decorators.size;

      setTimeout(function () {
        var newText = 'Yoyoyoyo dude';
        comp.setProps({ text: newText });
        (0, _chai.expect)(comp.state('editorState').getDecorator().decorators.size).to.eq(decoratorNumber);
        (0, _chai.expect)(comp.state('editorState').getCurrentContent().getPlainText()).to.eq(newText);
        done();
      }, 100);
    });
  });
